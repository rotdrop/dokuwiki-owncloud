            ...
            <div id="dokuwiki__content"><div class="pad group">
                ...
                <div class="page group">
                    ...
                    if($ERROR):
						...
						$path = $helper->wikiIDToPath($IMG);
						$dir = dirname($path);
                        $isDir = $helper->isMediaDir($path);
                        
                        if(!$isDir) echo nl2br(hsc(tpl_img_getTag('simple.title'))); 
                        else echo nl2br(hsc('/'.$path)); 
                        ?></h1>
                        ...
                        if(!$isDir){
							list($ext,$mime,$dl) = mimetype(noNS($IMG),false);
							if(substr($mime,0,5) == 'image'){
								tpl_img(900,700); /* parameters: maximum width, maximum height (and more) */ 
							}else{
								echo $helper->internalmedia($_GET['fileid'],$IMG,$helper->getLang('download'),NULL,NULL,NULL,NULL,'direct')."<br/><br/>";
                            ...
                            <dl>
                                <?php
									...
									if(isset($_GET['fileid'])){
											...
											$fileid = $helper->getIDForFilename($helper->wikiIDToPath($IMG));
									}
									if(!$isDir){
										...
										if(isset($type) && !empty($type)) echo "<dt>".$lang['img_format'].":</dt><dd>$type</dd>";
										echo "<dt>".$helper->getLang('path').':</dt><dd><a href="'.$helper->getConf('owncloudurl').'/index.php/apps/files?dir=//'.$helper->wiki.'/'.$dir.'"target="_blank">/'.$path.'</a>&nbsp;&nbsp(';
									}else{
                        ...
                        <?php
                        if(!$isDir){
							echo $helper->mediaMetaStart($path);
							echo $helper->mediaInUse($fileid);
						}
						
                        ?>
                        <?php   //dbg(tpl_img_getTag('Simple.Raw'));?>
                    <?php endif; ?>
                </div>
                <!-- detail stop -->
